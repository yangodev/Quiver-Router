	...
	function Server(req, res, done) {
		...
		var match = req.route = table.get().reduce(
			function (found, route, index, routes) {
				...
				if (!found) {
					return (route.match(req)) ? route : false
				}
			},
			false)
		...
		handler.context({
			...
			.finally(function (err, msg) {
				if (err) {
					...
					return done(null,
						'Message: internal error\n' + 'Error: ' + err + 'Additional: ' + (msg || this.msg || '----') + 'Action: ' + 'Redirect to /errors/500'
					)
				}
				if (this.served) {
					return done(null,
					...
					return done(null,
						'Error: Route ' + req.url.pathname + ' not Found\n' + 'Additional: ' + (msg || this.msg || '----') + 'Action: Redirect to /errors/404'
					)
				}
			})
