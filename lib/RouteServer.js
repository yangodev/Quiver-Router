...
function setup(options, table) {
	...
	function Server(req, res, done) {
		...
		.finally( function (err, msg) {
			if (err) {
				var route = table.id('qvr.errors.' + String(err.id)) || table.id('qvr.errors.500')
				...
				return done(null, {
					...
					Route		: match,
					Additional	: this.msg || '',
					Action		: 'Redirect to /errors/' + err.id || 500
				})
			}
			else if (this.served) {
				return done(null,{
				...
				res.redirect(table.id('qvr.errors.404').reverse({route:req.uri.pathname}) )
				return done(null, {
					Error		: 'Not Found',
					url			: req.uri.pathname,
					Additional	: msg || this.msg || '',
	...
	function _static(route) {
		return function (req, res, next) {
			var self = this
			var base = (route.options) ? route.options.path || '/' : '/'
