...
/* Utils */
var utils = require('yangodev.utils')
,	merge = utils.merge
,	path = require('path')
,	fs = require('fs')
/*
...
var	createDefinition = function create(route){
	...
	var routeAPI = {merge 	: _extend,
					match 	: _match}

	route = merge( wrapper, definition, routeAPI )
	setMethods()
	setMiddleware()
	if (route.type === 'static' ) { onStatic() }
		...
		function nextFragment(){
			...
			if(_cur.param[7] && _cur.param[7] !== 'undefined'){
				if (_cur.param[7] === _req.fragment){
					...
					,	lookup_padding = 0
					,	lookup = function(){
						return _req.fragments[current_index
										 + lookup_padding]
					}
			...
			else if (_cur.param[2] && _cur.param[2] !== 'undefined'
					 || _cur.param[3] && _cur.param[3] !== 'undefined'){
				var paramContent = []
