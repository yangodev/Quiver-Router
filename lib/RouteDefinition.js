...
var createDefinition = function create(route) {
	...
	function _match(req) {
		...
		var out = merge({}, self)
		out.query 		= req.url.query
		out.hash		= req.url.hash
		out.auth		= req.url.auth
		out.method		= req.method
		out.cookies		= req.cookies
		...
		function nextFragment() {
			...
			if (_cur.param[4] !== undefined) {
				if (_cur.param[4] === _req.fragment) {
			 ...
			 */
			var paramID = (_cur.param[3] === undefined)
			? 'wildcard' : _cur.param[3]
			...
			if (_cur.param[1] === '...'){
				...
				while (lookup() !== lookup_end_value
					   && (current_index + lookup_padding) <= _req.path.length)
				{
				...
				_cur.paramcoll[paramID] = _cur.paramcoll[paramID]
				? [].concat(_cur.paramcoll[paramID], paramContent)
				: paramContent
	...
	function _reverse(data){
		...
		.map(function(fragment){
			 ...
			 * non-param
			 */
			if (fragment[4]) 				{ return fragment[4] }

			/**
			 * params
			 */
			var paramcontent = (fragment[3]) ? data[fragment[3]] : data['wildcard']
			if (!paramcontent) { missing.push (fragment[3] || 'wildcard') }

			if ( fragment[1] === '...' )	{ return paramcontent }
			if ( fragment[1] === '@' )		{ return paramcontent }
		...
		if (missing.length > 0){
			err = new Error('Data missing for param:'
							+ missing.join('\n *'))
			err.missing = missing
		}
